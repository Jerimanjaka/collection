...
const defaultReasons = [
  { title: "Global Network", desc: "Access to a worldwide network of artisans, manufacturers, and designers." },
  { title: "Quality Standards", desc: "Uncompromising quality control at every stage of production." },
  { title: "Creative Freedom", desc: "Work on unique, bespoke projects for leading luxury brands." },
  { title: "Ethical Practices", desc: "Commitment to sustainable sourcing and fair trade principles." },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div>
            <div className="flex items-center gap-4 mb-4">
              ...
              <span className="font-[var(--font-inter)] text-xs tracking-[0.3em] uppercase text-champagne">
                <Text k="join.subtitle" fallback="Careers & Partnerships" />
              </span>
            </div>
            ...
            <p className="font-[var(--font-inter)] text-sm text-grey leading-relaxed mb-10 max-w-lg">
              <Text k="join.description" fallback="We are always looking for talented individuals and partners who share our passion for excellence. Whether you are a craftsman, designer, or industry professional — there is a place for you at Premiere Collection." />
            </p>
            <a
              href="#contact"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {await Promise.all(defaultReasons.map(async (reason, index) => {
              const title = await getText(`join.reason${index}.title`, reason.title);
              const desc = await getText(`join.reason${index}.desc`, reason.desc);
              return (
                <div
                  key={index}
